  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'flip') {
        ...
        if (game.pendingResolve) return res.status(400).json({ error: '请先结算上一轮翻牌' })
        ...
        if (game.revealed.length === 2) {
          // Keep both revealed so client can see them; mark pending
      ...
      if (action === 'resolve') {
        // Called after a delay to resolve the 2 revealed cards
        const row = db.prepare("SELECT value FROM kv WHERE key = 'pool_match'").get()
        if (!row) return res.status(400).json({ error: '没有进行中的游戏' })
        const game = JSON.parse(row.value)
        if (!game.pendingResolve || game.revealed.length !== 2) {
          return res.status(400).json({ error: '没有待结算的翻牌' })
        }
        const [a, b] = game.revealed
        const isMatch = game.board[a] === game.board[b]
        ...
        if (isMatch) {
          ...
          if (game.matched.length >= 16) {
            ...
            return res.json({ game: clientGame(game), matched: true, gameOver: true, stats: getStats(db) })
          }
        ...
        db.prepare('INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, unixepoch())').run('pool_match', JSON.stringify(game))
        return res.json({ game: clientGame(game), matched: isMatch, stats: getStats(db) })
      }
