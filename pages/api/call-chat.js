...
// Wraps /api/chat with SSE sentence splitting for TTS
// Reuses all tools, MCP, system prompt from regular chat
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Clean up text for TTS (remove img tags, voice tags etc)
    ...
    for (const sentence of sentences) {
      if (sentence.trim()) {
        res.write(`data: ${JSON.stringify({ type: 'sentence', text: sentence.trim() })}\n\n`)
      }
    }
