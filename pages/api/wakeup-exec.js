...
async function executeTool(name, args) {
  ...
  if (name === 'write_note') {
    if (!args.text || !String(args.text).trim()) return { success: false, message: '便签内容不能为空' }
    ...
    page.notes.push({ id: 'n_' + Date.now(), text: noteText, paper: args.paper !== undefined ? args.paper : Math.floor(Math.random() * 6), x: 20 + Math.random() * 100, y: 20 + Math.random() * 100, rot: (Math.random() - 0.5) * 8 })
    state.currentPage = state.pages.length - 1
    db.prepare('INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, unixepoch())').run(key, JSON.stringify(state))
    return { success: true, message: '便签已写入: "' + noteText + '"' }
  }
  ...
  if (name === 'reader_read_chapter') {
    try {
      ...
      return { title: chapter.title, content, chapterIndex: ch, totalChapters: book.chapters.length, totalChars: fullContent.length, truncated, aiChapter: state.aiChapter }
    } catch (e) { return { error: e.message } }
  }

  if (name === 'reader_add_note') {
    try {
      // Always use actual current book ID from state, not AI's (often wrong) book_id
      const stRow = db.prepare("SELECT value FROM kv WHERE key = 'pool_reader_state'").get()
      const rs = stRow ? JSON.parse(stRow.value) : {}
      const actualBookId = rs.currentBookId || args.book_id
      const notesRow = db.prepare("SELECT value FROM kv WHERE key = 'pool_reader_notes'").get()
      const notes = notesRow ? JSON.parse(notesRow.value) : []
