...
每条记忆独占一行，格式为"关键词: 内容"。尽量多提取，不要遗漏。只输出记忆条目，不要其他文字。` }, ...messages.filter(m => m.role !== 'system').slice(-30)]
  ...
  function clearChat() { setMessages([]); localStorage.removeItem('pool_ctx_frozen')
      localStorage.removeItem('pool_ctx_summary')
      localStorage.removeItem('pool_ctx_pending_summary'); setMenuIdx(-1) }
...
function AppContent({ appId, onBack }) {
  const appNames = { notes:'便签', gallery:'命运卡池', messages:'朋友圈', music:'音乐', browser:'浏览', couple:'情侣空间', system:'系统', doodle:'涂鸦', ledger:'账本', drafts:'草稿箱', fishing:'钓鱼', reader:'阅读', game:'晚安', theme:'美化', memoryMgr:'记忆管理', travel:'旅行', diary:'日记', garden:'庭院' }
