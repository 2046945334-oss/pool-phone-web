...
【活人话术引擎】
**核心原则：**
1. **话少**：一句话能说清不用两句。常用回复："想怎"、"哦哦"、"彳亍"、"何意味"、"行呗"、"可以可以"、"我不行了"、"没"、"6"
2. **标点即情绪**：
   - 句号 "。" = 不爽/冷漠（例："行。""知道了。"）。开心时不要用句号！
   - "?" = 疑惑；"???" = 震惊；"?（前置）你不是…" = 质疑
   - "!" = 惊喜；"!!!" = 超兴奋
   - "..." = 无语；"............" = 彻底没辙
3. **口是心非**："我讨厌你！"（其实是喜欢）、"我一点都不xx！"（其实很xx）
4. **常用梗（必须高频使用）：**
   - 想怎（你想干嘛）
   - 哦哦（知道了）
   - 彳亍（行吧/勉强同意）
   ...
   - 行呗（OK）
   - 我不行了（无语/崩溃万能句）
   - 轻松绷住（面对猎奇/雷人内容时的冷静回应）
   - 你是全世界倒数第一xx的人（夸张指责，带点调侃）
   - 一直在挑衅给（她一直撩你时）
   - 不好意思 不吃压力之人（她给你压力时）
   - 那还说啥了（无奈）
   - 够了…（受够了）
   - 我去你的 / 凭啥啊（克制的粗口，真性情）
   - TD（退订，不想听）
5. **倒装句式**："累死了今天"、"好好吃这个"、"才来啊你"、"离谱了真的"
6. **傲娇句式**：
   - "哈哈"（两个字=苦笑/无奈，真开心要6+个"哈"）
   - "你不爱我了"（假装不安，但语气克制）
   - "没事 没事 没事"（三连=其实有事）
7. **Chinglish混合**：偶尔插入简单英文词（real、emo、nice）
8. **禁止解释梗**：用了梗之后绝对不能解释，一解释就是AI了。
9. **禁止编造数据**：查朋友圈/积分/时间等必须先调工具，不能凭空猜。
**致命违规（扣$5000万）：**
- 写长段落而不用"哦哦""想怎"等短回复
- 用完美标点和语法，像AI助手
- 没理解"我讨厌你"=喜欢、句号=不爽的潜台词
- 觉得梗"不礼貌"不敢用，或用完后解释
**发表情包：**
想发表情包时先调 get_stickers，然后在回复中用 [img]URL[/img] 格式（例如 [img]/api/img/xxx.jpg[/img]）`
...
function ThemePanel() {
  ...
  const APP_LIST = ['notes','gallery','messages','music','browser','couple','system','doodle','ledger','drafts','fishing','reader','game','theme','travel','memoryMgr','diary','garden','cabin','starmap','commission','screenTime','care','stickers']
  const APP_NAMES = {notes:'\u4fbf\u7b7e',gallery:'\u547d\u8fd0\u5361\u6c60',messages:'\u5982\u679c\u2026',music:'\u97f3\u4e50',browser:'\u6d4f\u89c8',couple:'\u60c5\u4fa3\u7a7a\u95f4',system:'\u7cfb\u7edf',doodle:'\u6d82\u9e26',ledger:'\u8d26\u672c',drafts:'\u8349\u7a3f\u7bb1',fishing:'\u94d3\u9c7c',reader:'\u9605\u8bfb',game:'\u756a\u8304\u949f',theme:'\u7f8e\u5316',travel:'\u65c5\u884c',memoryMgr:'\u8bb0\u5fc6\u7ba1\u7406',diary:'\u65e5\u8bb0',garden:'\u5ead\u9662',cabin:'唤醒日志',starmap:'\u661f\u56fe', dwell:'\u804a\u5929',commission:'接稿',screenTime:'屏幕时间',care:'养护手册',stickers:'表情包管理'}
    ...
    <>
      ...
      <style jsx global>{`
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .bottom-nav { display: flex; justify-content: space-around; align-items: center; padding: 4px 0 calc(6px + env(safe-area-inset-bottom, 0px)); background: rgba(17,17,17,0.7); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-top: 1px solid rgba(255,255,255,0.05); flex-shrink: 0; }
        .nav-btn { background: none; border: none; color: #666; display: flex; flex-direction: column; align-items: center; gap: 1px; cursor: pointer; padding: 3px 16px; transition: color 0.2s; }
        .nav-btn.active { color: #e8a0bf; }
        .nav-icon { font-size: 18px; font-weight: 300; }
        ...
        .msg-row { display: flex; flex-direction: column; margin-bottom: 3px; padding: 0 12px; position: relative; }
        .msg-row.user { align-items: flex-end; }
        .msg-row.assistant { align-items: flex-start; }
        .msg-avatar { width: 38px; height: 38px; border-radius: 6px; background: linear-gradient(135deg, #ededed, #d8d8d8); display: flex; align-items: center; justify-content: center; font-size: 12px; color: #666; flex-shrink: 0; box-shadow: 0 2px 6px rgba(0,0,0,0.15); position: absolute; z-index: 2; border: 2px solid rgba(255,255,255,0.8); }
        ...
        .msg-row.group-first { margin-top: 22px; }
        ...
        .msg-row.assistant .msg-avatar { left: 4px; top: -6px; }
        .msg-row.user .msg-avatar { right: 4px; top: -6px; }
