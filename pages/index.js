    ...
    <div className="chat-view">
      <div className="chat-header">
        ...
        <div style={{marginLeft:'auto',display:'flex',gap:'8px'}}>
          ...
          <button onClick={() => { const on = !overlayOn; setOverlayOn(on); if (window.ChiOverlay) { on ? window.ChiOverlay.startOverlay() : window.ChiOverlay.stopOverlay() } }} onContextMenu={(e) => { e.preventDefault(); setOverlayAvatarModal(true); fetch('/api/data/pool_overlay_config').then(r=>r.json()).then(d => { try { const cfg = JSON.parse(d.value || '{}'); setOverlayAvatarUrl(cfg.bubbleAvatar || '') } catch {} }).catch(()=>{}) }} style={{background: overlayOn ? 'rgba(200,125,186,0.25)' : 'none', border: overlayOn ? '1px solid rgba(200,125,186,0.5)' : 'none', color: overlayOn ? '#c77dba' : '#9a8a99', cursor:'pointer', padding:'4px', borderRadius:'4px'}} title={'\u60ac\u6d6e\u7a97\uff08\u957f\u6309\u8bbe\u7f6e\u5934\u50cf\uff09'}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="10" r="3"/><path d="M6.168 18.849A4 4 0 0 1 10 16h4a4 4 0 0 1 3.834 2.855"/></svg></button>
        </div>
      </div>
      ...
      <div className="chat-messages" style={theme?.chatBg ? {backgroundImage:`url(${theme.chatBg})`,backgroundSize:'cover',backgroundPosition:'center'} : {}} onClick={() => setMenuIdx(-1)}>
        ...
        {messages.slice(visibleStart).map((msg, idx) => {
          ...
          if (msg.isReadingSync || msg.isGameSync || msg.isWatchSync || msg.hidden) return null
          return (
    ...
    <>
      ...
      <div className="shell">
        <div className="phone-frame">
          ...
          <div className="bottom-nav-wrap" style={{display: (currentApp || activeTab === 'chat') ? 'none' : 'flex'}}>
...
.hs-tv-card { margin-bottom: 8px; cursor: pointer; transition: transform 0.15s; }
        ...
        .msg-bubble.sticker-only { background: transparent !important; backdrop-filter: none !important; -webkit-backdrop-filter: none !important; border: none !important; box-shadow: none !important; padding: 0 !important; }
        .chat-input-area { display: flex; align-items: center; gap: 2px; padding: 5px 6px; background: rgba(255,230,245,0.35); backdrop-filter: blur(28px); -webkit-backdrop-filter: blur(28px); border: 1px solid rgba(255,220,240,0.3); border-radius: 28px; position: absolute; bottom: calc(8px + env(safe-area-inset-bottom, 0px)); left: 8px; right: 8px; z-index: 10; box-shadow: 0 4px 20px rgba(0,0,0,0.06); pointer-events: auto; overflow: visible; }
        .chat-plus-btn { width: 28px; height: 28px; border-radius: 50%; background: #fff; color: #333; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 15px; flex-shrink: 0; border: 1px solid rgba(0,0,0,0.08); box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
