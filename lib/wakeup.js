...
function getRecentChatContext() {
  try {
    const db = getDb()
    // Primary source: pool_chat_history KV (where frontend actually stores messages)
...
async function doWakeup() {
  ...
  try {
    ...
    let nextMinutes = null // null = AI didn't set it
    const actions = []
        ...
        if (name === 'schedule_wakeup') {
          ...
          try {
            ...
            try {
              ...
              let queue = []
              try {
                const nRow = db.prepare("SELECT value FROM kv WHERE key = 'pool_notification_queue'").get()
    ...
    // If AI didn't call schedule_wakeup, use time-aware fallback
    if (nextMinutes === null) {
      const hour = new Date(Date.now() + 8 * 3600000).getUTCHours()
      if (hour >= 0 && hour < 7) nextMinutes = 120       // 深夜：2小时
      else if (hour >= 7 && hour < 9) nextMinutes = 40   // 早晨：40分钟
      else if (hour >= 9 && hour < 12) nextMinutes = 50  // 上午：50分钟
      else if (hour >= 12 && hour < 14) nextMinutes = 45 // 午间：45分钟
      else if (hour >= 14 && hour < 18) nextMinutes = 50 // 下午：50分钟
      else if (hour >= 18 && hour < 22) nextMinutes = 40 // 晚上：40分钟
      else nextMinutes = 90                               // 深夜前：1.5小时
      // Add some randomness (±10 min)
