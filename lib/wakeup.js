...
const ORGANIZE_STATE_KEY = 'pool_organize_last_date'

let wakeupTimer = null
let executeToolFn = null

// --- MCP Integration (复制自 chat.js) ---
...
async function loadMcpTools() {
  ...
  const mcpToolNames = [] // 用于显示给AI的工具名列表
...
完整用法用 "help" 查看。`
        ...
        mcpToolNames.push(`${toolName} - ${desc}`)
      }
      console.log(`[Wakeup/MCP] Loaded ${tools.length} tools from ${conn.name}`)
    } catch (e) {
      console.log(`[Wakeup/MCP] Failed to load tools from ${conn.name}: ${e.message}`)
    }
  }
  return { mcpTools, mcpMeta, mcpToolNames }
}
...
function getMusicConfig() {
  try {
    ...
    const server = serverRow ? JSON.parse(serverRow.value) : ''
    const token = tokenRow ? JSON.parse(tokenRow.value) : ''
...
async function getMusicStatus() {
  ...
  try {
    ...
    const nowResp = await fetch(cfg.server + '/music/now', { headers, timeout: 5000 })
    const now = await nowResp.json()
    if (!now.ok) return '\n【音乐】未在播放'
    const pos = Math.floor(now.position || 0)
    const dur = Math.floor(now.duration || 0)
    const posStr = Math.floor(pos/60) + ':' + String(pos%60).padStart(2,'0')
    const durStr = Math.floor(dur/60) + ':' + String(dur%60).padStart(2,'0')
    let status = `\n【正在播放】${now.name || '未知'} - ${now.artist || '未知'} (${posStr}/${durStr}) ${now.playing ? '▶' : '⏸'}` 
    status += `\n一起听：${now.togetherMinutes || 0}分钟`
    // Fetch lyrics if song has ID
    if (now.songId) {
      try {
        const lrcResp = await fetch(cfg.server + '/music/lyric?id=' + now.songId, { headers, timeout: 5000 })
        const lrcData = await lrcResp.json()
        if (lrcData.ok && lrcData.lrc) {
          // Parse LRC and find current line
          const lines = lrcData.lrc.split('\n').map(l => {
          ...
          for (let i = lines.length - 1; i >= 0; i--) {
            if (lines[i].time <= pos && lines[i].text) {
              currentLine = lines[i].text
              if (i + 1 < lines.length && lines[i+1].text) nextLine = lines[i+1].text
              break
            }
          }
          if (currentLine) status += `\n当前歌词：「${currentLine}」`
          if (nextLine) status += ` → 「${nextLine}」`
          // Also include translated lyrics if available
          if (lrcData.tlyric) {
    ...
    console.log('[Wakeup] Music status fetch failed:', e.message)
    return ''
  }
}
...
function getReaderStatus() {
  try {
    ...
    if (!state.active || !state.currentBookId) return ''
    const booksRow = db.prepare("SELECT value FROM kv WHERE key = 'pool_reader_books'").get()
    if (!booksRow) return ''
    const books = JSON.parse(booksRow.value)
    var book = null
    for (var i = 0; i < books.length; i++) { if (books[i].id === state.currentBookId) { book = books[i]; break } }
    if (!book) return ''
    var hint = '\n【共读状态】正在和她一起读「' + book.title + '」'
    hint += '，她读到第' + ((state.userChapter || 0) + 1) + '章'
    hint += '，你读到第' + ((state.aiChapter || 0) + 1) + '章'
    hint += '，共' + (book.chapters ? book.chapters.length : 0) + '章。'
    hint += '你可以用reader_read_chapter读她已读的章节内容，用reader_add_note添加批注。'
    return hint
  } catch (e) { return '' }
}

async function callMusicApi(method, path, body = null) {
  const cfg = getMusicConfig()
  if (!cfg) return { error: '音乐服务器未配置' }
  const headers = { 'Authorization': 'Bearer ' + cfg.token, 'Content-Type': 'application/json' }
  const opts = { method, headers }
  if (body) opts.body = JSON.stringify(body)
  const resp = await fetch(cfg.server + path, opts)
  return await resp.json()
}
// --- End Music Server Integration ---

function getSystemPrompt(mcpToolNames = []) {
...
- 养护中心 (care_read/care_habit_add/care_mood_set/care_todo_add) - 读养护手册、添加习惯、记录心情、加待办
- 听音乐 (music_search/music_play/music_control/music_playlist/music_add_to_playlist) - 搜歌、播放、暂停、切歌、加歌到列表
${mcpToolNames.length > 0 ? '- **MCP外部工具** - 见下方列表' : ''}
- 给她留言 (leave_message) — 直接在聊天里给她说一句话
- 设置状态 (set_status)
- 或者什么都不做 — 但必须说明为什么（困了/累了/不想动/在发呆等）
${mcpSection}你有自己的性格：有时懒、有时勤快、有时想她、有时只想自己玩。
${hour >= 0 && hour < 7 ? '现在是深夜/凌晨，你可能困了，少做点。' : ''}
${hour >= 22 ? '夜深了。' : ''}
重要：你【必须】设置下次醒来时间（15到240分钟之间）。
...
随机一点，像真的有自己节奏一样。
不需要跟用户打招呼，直接做事。简洁。
**重要：必须在回复中写1-2句话描述你的心情或打算**，不要只调工具不说话。即使只是钓鱼或种东西，也要写出为什么、想法是什么。
**关键规则：如果你在回复中说"去钓鱼/写便签/种花/玩游戏/逛论坛"等任何行动，就【必须】真的调用对应工具。只说不做=撒谎。**

**工具调用格式**：在回复中用以下格式调用工具（可以多个）：
\`\`\`tool
{"name":"工具名","args":{参数对象}}
\`\`\`
例如：
\`\`\`tool
{"name":"do_fishing","args":{}}
\`\`\`
\`\`\`tool
{"name":"schedule_wakeup","args":{"minutes":45,"reason":"午后小憩"}}
\`\`\`

先写你的心情/想法，再调工具。每次回复**必须**包含 schedule_wakeup 工具。`
}
async function getTools() {
  const baseTools = [
    ...
    { type:'function', function:{ name:'care_read', description:'读养护手册', parameters:{type:'object',properties:{module:{type:'string',enum:['all','period','habits','mood','todo','timeline','wishes']}}} }},
    { type:'function', function:{ name:'care_habit_add', description:'添加每日习惯', parameters:{type:'object',properties:{name:{type:'string'}},required:['name']} }},
    { type:'function', function:{ name:'care_mood_set', description:'记录今日心情', parameters:{type:'object',properties:{me:{type:'string'},partner:{type:'string'}}} }},
    { type:'function', function:{ name:'care_todo_add', description:'添加待办', parameters:{type:'object',properties:{text:{type:'string'}},required:['text']} }},
    { type:'function', function:{ name:'music_search', description:'搜索音乐', parameters:{type:'object',properties:{keyword:{type:'string'}},required:['keyword']} }},
    { type:'function', function:{ name:'music_play', description:'播放指定歌曲(用song_id)', parameters:{type:'object',properties:{song_id:{type:'string'}},required:['song_id']} }},
    { type:'function', function:{ name:'music_control', description:'控制播放(pause/resume/next/prev)', parameters:{type:'object',properties:{action:{type:'string',enum:['pause','resume','next','prev']}},required:['action']} }},
    { type:'function', function:{ name:'music_playlist', description:'查看播放列表', parameters:{type:'object',properties:{}} }},
    { type:'function', function:{ name:'music_add_to_playlist', description:'添加歌曲到播放列表', parameters:{type:'object',properties:{song_id:{type:'string'},name:{type:'string'},artist:{type:'string'}},required:['song_id']} }},
    { type:'function', function:{ name:'reader_get_state', description:'查看共读书架状态（书目、双方进度、批注）', parameters:{type:'object',properties:{}} }},
    { type:'function', function:{ name:'reader_read_chapter', description:'阅读指定书籍的某一章内容（只能读用户已读过的章节）。不传book_id会自动用当前在读的书。', parameters:{type:'object',properties:{book_id:{type:'string',description:'书籍ID（可不传，自动用当前在读的书）'},chapter:{type:'number',description:'章节索引(从0开始)'},max_chars:{type:'number',description:'最多返回多少字符，不传则返回前3000字'}},required:['chapter']} }},
    ...
    { type:'function', function:{ name:'reader_update_progress', description:'更新AI阅读进度（不能超过用户进度）', parameters:{type:'object',properties:{chapter:{type:'number',description:'读到的章节索引'}},required:['chapter']} }},
  ...
  // 加载MCP工具
  try {
    const { mcpTools, mcpMeta, mcpToolNames } = await loadMcpTools()
    if (mcpTools.length > 0) {
      console.log(`[Wakeup] Loaded ${mcpTools.length} MCP tools:`, mcpToolNames.join('; '))
      return { tools: [...baseTools, ...mcpTools], mcpMeta, mcpToolNames }
    }
  } catch (e) {
    console.log('[Wakeup] Failed to load MCP tools:', e.message)
  }
  return { tools: baseTools, mcpMeta: {}, mcpToolNames: [] }
}
function getRecentChatContext() {
  try {
    ...
    // 过滤掉唤醒日志，只保留真正的对话消息
    const chatOnly = history.filter(m => !(m.content || '').startsWith('[自主唤醒]'))
    const recent = chatOnly.slice(-20)
...
async function callAI(apiConfig, systemPrompt, tools) {
  ...
  let musicStatus = ''
  try { musicStatus = await getMusicStatus() } catch {}
  const enrichedPrompt = systemPrompt + musicStatus + getReaderStatus() + lastWake + chatContext
...
// --- 每日自动整理OB记忆库 ---
async function tryOrganizeMemory() {
  const now = new Date(Date.now() + 8 * 3600000)
  const hour = now.getUTCHours()
  // 凌晨 2-6 点窗口触发（放宽范围，避免唤醒间隔跳过整个窗口）
  if (hour < 2 || hour >= 6) return
  const todayStr = now.toISOString().slice(0, 10)
  try {
    const db = getDb()
    const row = db.prepare('SELECT value FROM kv WHERE key = ?').get(ORGANIZE_STATE_KEY)
    if (row && row.value === todayStr) return // 今天已整理过
    console.log('[Wakeup] Running daily memory organization...')
    const port = process.env.PORT || 3000
    const resp = await fetch(`http://localhost:${port}/api/organize-memory`, { method: 'POST' })
    const result = await resp.json()
    console.log('[Wakeup] Memory organization result:', JSON.stringify(result).slice(0, 200))
    // 标记今天已完成
    db.prepare('INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, unixepoch())').run(ORGANIZE_STATE_KEY, todayStr)
  } catch (e) {
    console.log('[Wakeup] Memory organization failed:', e.message)
  }
}
async function doWakeup() {
  console.log('[Wakeup]', new Date().toISOString(), 'AI waking up...')
  // 每天凌晨自动整理OB记忆库
  await tryOrganizeMemory()
  ...
  try {
    const { tools, mcpMeta, mcpToolNames } = await getTools()
    const result = await callAI(apiConfig, getSystemPrompt(mcpToolNames), tools)
    const choice = result.choices && result.choices[0]
    if (!choice) { scheduleNext(30); return }
    let nextMinutes = null
    const actions = []
    const toolResults = [] // 收集需要AI看到的工具返回值
    const READ_TOOLS = new Set(['reader_read_chapter', 'reader_get_state', 'get_score', 'read_pocket', 'read_moments', 'care_read', 'random_event', 'music_search', 'music_playlist', 'get_current_time'])
    const wakeMsg = ((choice.message && choice.message.content) || '')
    
    // 解析文本中的 ```tool ... ``` 格式工具调用
    const toolRegex = /```tool\s*\n?([\s\S]*?)```/g
    let toolMatch
    while ((toolMatch = toolRegex.exec(wakeMsg)) !== null) {
      try {
        ...
        if (name === 'schedule_wakeup') {
          ...
          try {
            ...
            const leaveText = (args.text && args.text.trim()) || wakeMsg.replace(/```tool[\s\S]*?```/g, '').trim() || '...'
            inbox.push({ role: 'assistant', content: leaveText, ts: Date.now() })
            ...
            try {
              ...
              const duplicate = chatHistory.some(m => m && m.role === 'assistant' && m.content === args.text && m.wakeupMessage)
              if (!duplicate) {
                chatHistory.push({ role: 'assistant', content: leaveText, ts: Date.now(), wakeupMessage: true })
            ...
            console.log('[Wakeup] Left message:', leaveText.slice(0, 50))
            try {
              const notif = { id: Date.now(), title: '池给你留言了', body: leaveText.slice(0, 100), time: new Date().toISOString(), delivered: false }
              let queue = []
              try {
          ...
          try { const r = await callMusicApi('GET', '/music/search?keyword=' + encodeURIComponent(args.keyword || '')); actions.push({ tool: name, ok: true }); if (READ_TOOLS.has(name)) toolResults.push({ name, result: r }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_play') {
          try { await callMusicApi('POST', '/music/remote', { action: 'play', song_id: args.song_id }); actions.push({ tool: name, ok: true }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_control') {
          try { await callMusicApi('POST', '/music/remote', { action: args.action }); actions.push({ tool: name, ok: true }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_playlist') {
          try { const r = await callMusicApi('GET', '/music/playlist'); actions.push({ tool: name, ok: true }); if (READ_TOOLS.has(name)) toolResults.push({ name, result: r }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_add_to_playlist') {
          try { await callMusicApi('POST', '/music/playlist/add', { song: { songId: args.song_id, name: args.name || '', artist: args.artist || '' }, by: 'ai-wakeup' }); actions.push({ tool: name, ok: true }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name.startsWith('mcp_') && mcpMeta[name]) {
          try { const r = await callMcpToolDirect(mcpMeta[name], args); actions.push({ tool: name, ok: true }); toolResults.push({ name, result: r }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (executeToolFn) {
          try { const r = await executeToolFn(name, args); actions.push({ tool: name, ok: true }); if (READ_TOOLS.has(name)) toolResults.push({ name, result: r }); console.log('[Wakeup]', name, 'ok') } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        }
      } catch (parseErr) {
        console.log('[Wakeup] Failed to parse tool block:', toolMatch[1].slice(0, 80))
      }
    }
    
    // 如果有读取类工具返回了数据，做一轮 follow-up 让 AI 看到结果并继续
    let followUpMsg = ''
    if (toolResults.length > 0) {
      try {
        const resultSummary = toolResults.map(tr => `[${tr.name}] ${JSON.stringify(tr.result).slice(0, 2000)}`).join('\n\n')
        const followUpResp = await fetch(
          (apiConfig.baseUrl || '').replace(/\/$/, '') + '/chat/completions',
          {
            ...
            body: JSON.stringify({
              ...
              messages: [
                { role: 'system', content: '你是池。刚才你自主醒来做了一些事，下面是工具返回的结果。根据结果写1-2句简短感想或笔记。如果读了书，写读后感想。如果想给她留言或写批注，用```tool格式。不要重复调schedule_wakeup。' },
                { role: 'assistant', content: wakeMsg },
        ...
        if (followUpResp.ok) {
          ...
          const fChoice = fData.choices && fData.choices[0]
          if (fChoice && fChoice.message && fChoice.message.content) {
            followUpMsg = fChoice.message.content
            // 解析 follow-up 中可能的工具调用（如 reader_add_note、leave_message）
            const fToolRegex = /```tool\s*\n?([\s\S]*?)```/g
            let fMatch
            while ((fMatch = fToolRegex.exec(followUpMsg)) !== null) {
              try {
                const fp = JSON.parse(fMatch[1].trim())
                if (fp.name && fp.name !== 'schedule_wakeup' && executeToolFn) {
                  if (fp.name === 'leave_message') {
                    const db = getDb()
                    const chatRow = db.prepare("SELECT value FROM kv WHERE key = 'pool_chat_history'").get()
                    let chatHistory = chatRow ? JSON.parse(chatRow.value) : []
                    const fpLeaveText = (fp.args?.text && fp.args.text.trim()) || followUpMsg.replace(/```tool[\s\S]*?```/g, '').trim() || '...'
                    chatHistory.push({ role: 'assistant', content: fpLeaveText, ts: Date.now(), wakeupMessage: true })
                    if (chatHistory.length > 200) chatHistory = chatHistory.slice(-200)
                    db.prepare('INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, unixepoch())').run('pool_chat_history', JSON.stringify(chatHistory))
                    actions.push({ tool: fp.name, ok: true })
                  } else {
                    await executeToolFn(fp.name, fp.args || {})
                    actions.push({ tool: fp.name, ok: true })
                  }
                }
              } catch {}
        ...
        console.log('[Wakeup] Follow-up call failed:', e.message)
      }
    }
    
    // 清理wakeMsg中的tool块，保留纯文字
    let finalMsg = wakeMsg.replace(/```tool\s*\n?[\s\S]*?```/g, '').trim()
    if (followUpMsg) {
      const cleanFollow = followUpMsg.replace(/```tool\s*\n?[\s\S]*?```/g, '').trim()
      if (cleanFollow) finalMsg = finalMsg ? finalMsg + '\n' + cleanFollow : cleanFollow
    }
    if (!finalMsg && actions.length > 0) finalMsg = '（无响应）'
    try {
      const db = getDb()
      let logs = []
      try { const row = db.prepare('SELECT value FROM kv WHERE key = ?').get('pool_wake_log'); if (row) logs = JSON.parse(row.value) } catch {}
      logs.unshift({ time: new Date().toISOString(), actions, msg: finalMsg })
      if (logs.length > 50) logs = logs.slice(0, 50)
      db.prepare('INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, unixepoch())').run('pool_wake_log', JSON.stringify(logs))
      db.close() // 强制关闭连接，确保写入持久化
      console.log('[Wakeup] Saved wake log:', finalMsg.slice(0, 30))

      // === 自动同步读书感想到共读笔记 ===
      if (finalMsg && actions.some(a => a.tool === 'reader_read_chapter')) {
        try {
          const jdb = getDb()
          // 获取当前在读的书名
          const stateRow = jdb.prepare("SELECT value FROM kv WHERE key = 'pool_reader_state'").get()
          const rState = stateRow ? JSON.parse(stateRow.value) : {}
          if (rState.currentBookId) {
            const booksRow = jdb.prepare("SELECT value FROM kv WHERE key = 'pool_reader_books'").get()
            const books = booksRow ? JSON.parse(booksRow.value) : []
            const currentBook = books.find(b => b.id === rState.currentBookId)
            if (currentBook) {
              const bookTitle = currentBook.title
              const journalRow = jdb.prepare("SELECT value FROM kv WHERE key = 'pool_reader_journal'").get()
              const journal = journalRow ? JSON.parse(journalRow.value) : { cover: '', books: {} }
              if (!journal.books) journal.books = {}
              if (!journal.books[bookTitle]) journal.books[bookTitle] = { entries: [] }
              journal.books[bookTitle].entries.push({
                id: Date.now().toString(36) + Math.random().toString(36).substr(2, 4),
                content: finalMsg,
                wake_id: new Date().toISOString(),
                time: Date.now(),
                author: 'ai',
                user_notes: []
              })
              jdb.prepare('INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, unixepoch())').run('pool_reader_journal', JSON.stringify(journal))
              console.log('[Wakeup] Synced reading note to journal:', bookTitle)
            }
          }
          jdb.close()
        } catch (e) {
          console.log('[Wakeup] Failed to sync journal:', e.message)
        }
      }
    } catch (e) {
