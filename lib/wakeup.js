    ...
    if (choice.message && choice.message.tool_calls && executeToolFn) {
      for (const tc of choice.message.tool_calls) {
        ...
        if (name === 'schedule_wakeup') {
          ...
          try {
            const db2 = getDb()
            const chatRow = db2.prepare("SELECT value FROM kv WHERE key = 'pool_chat_history'").get()
            let chatHistory = chatRow ? JSON.parse(chatRow.value) : []
            chatHistory.push({ role: 'assistant', content: args.text || '...' })
            if (chatHistory.length > 200) chatHistory = chatHistory.slice(-200)
            db2.prepare('INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, unixepoch())').run('pool_chat_history', JSON.stringify(chatHistory))
