...
- 旅行 (get_travel_data/buy_travel_item) - 查看旅行记录或购买纪念品
...
async function getTools() {
  const baseTools = [
    ...
    { type:'function', function:{ name:'buy_travel_item', description:'购买旅行纪念品', parameters:{type:'object',properties:{item_id:{type:'string'}},required:['item_id']} }},
    { type:'function', function:{ name:'get_travel_data', description:'查旅行数据', parameters:{type:'object',properties:{}} }},
...
async function callAI(apiConfig, systemPrompt, tools) {
  ...
  const enrichedPrompt = systemPrompt + musicStatus + lastWake + chatContext
