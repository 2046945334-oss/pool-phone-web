...
async function doWakeup() {
  ...
  try {
    ...
    // Save log
    const wakeMsg = (choice.message.content || '').slice(0, 150)
    try {
      const db = getDb()
      let logs = []
      ...
      // 把唤醒行为写入聊天记录，这样下次唤醒/聊天时能看到
      try {
        db.exec("CREATE TABLE IF NOT EXISTS chat_sessions (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT DEFAULT '新对话', summary TEXT DEFAULT '', created_at INTEGER DEFAULT (unixepoch()), updated_at INTEGER DEFAULT (unixepoch()))")
        db.exec("CREATE TABLE IF NOT EXISTS chat_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id INTEGER DEFAULT 1, role TEXT NOT NULL, content TEXT NOT NULL, msg_type TEXT DEFAULT 'text', created_at INTEGER DEFAULT (unixepoch()))")
        let sessionId = 1
        try {
          const sess = db.prepare('SELECT id FROM chat_sessions ORDER BY updated_at DESC LIMIT 1').get()
        ...
        db.prepare('INSERT INTO chat_messages (session_id, role, content) VALUES (?, ?, ?)').run(sessionId, 'assistant', logContent)
      } catch (e) {
    ...
    try {
      ...
      if (mcpRow) {
        ...
        if (obConn) {
          ...
          // 调 MCP 的 tools/call 方法执行 I（写入记忆）
          ...
          const initResp = await fetch(obConn.url, { method: 'POST', headers, body: JSON.stringify(initBody) })
          let sid = null
          if (initResp.ok) {
            const initData = await initResp.json()
            sid = initData.sessionId || initResp.headers?.get?.('mcp-session-id') || null
          }
          // 发 notifications/initialized
          const notifHeaders = { ...headers }
          if (sid) notifHeaders['Mcp-Session-Id'] = sid
          await fetch(obConn.url, { method: 'POST', headers: notifHeaders, body: JSON.stringify({ jsonrpc: '2.0', method: 'notifications/initialized' }) }).catch(() => {})
          // 调 tools/call -> I
          const callHeaders = { ...headers }
          if (sid) callHeaders['Mcp-Session-Id'] = sid
          const callBody = { jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: 'I', arguments: { text: memContent } } }
          await fetch(obConn.url, { method: 'POST', headers: callHeaders, body: JSON.stringify(callBody) }).catch(() => {})
