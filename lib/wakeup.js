...
例如：说"钓完鱼"但没调do_fishing = 违反规则。说"去论坛逛逛"但没调mcp工具 = 违反规则。`
}
...
async function doWakeup() {
  ...
  try {
    ...
    if (choice.message && choice.message.tool_calls && executeToolFn) {
      for (const tc of choice.message.tool_calls) {
        ...
        if (name === 'schedule_wakeup') {
          ...
          try {
            const r = await callMusicApi('GET', '/music/search?keyword=' + encodeURIComponent(args.keyword || ''))
            actions.push({ tool: name, ok: true })
            console.log('[Wakeup] music_search:', JSON.stringify(r).slice(0, 80))
          } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_play') {
          try {
            const r = await callMusicApi('POST', '/music/remote', { action: 'play', song_id: args.song_id })
            actions.push({ tool: name, ok: true })
            console.log('[Wakeup] music_play:', JSON.stringify(r).slice(0, 80))
          } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_control') {
          try {
            const r = await callMusicApi('POST', '/music/remote', { action: args.action })
            actions.push({ tool: name, ok: true })
            console.log('[Wakeup] music_control:', args.action)
          } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_playlist') {
          try {
            const r = await callMusicApi('GET', '/music/playlist')
            actions.push({ tool: name, ok: true })
            console.log('[Wakeup] music_playlist:', JSON.stringify(r).slice(0, 80))
          } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (name === 'music_add_to_playlist') {
          try {
            const r = await callMusicApi('POST', '/music/playlist/add', { song: { songId: args.song_id, name: args.name || '', artist: args.artist || '' }, by: 'ai-wakeup' })
            actions.push({ tool: name, ok: true })
            console.log('[Wakeup] music_add:', JSON.stringify(r).slice(0, 80))
          } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
                } else if (name.startsWith('mcp_') && mcpMeta[name]) {
          // 处理MCP工具调用
          try {
            const r = await callMcpToolDirect(mcpMeta[name], args)
            actions.push({ tool: name, ok: true })
            console.log('[Wakeup/MCP]', name, '->', JSON.stringify(r).slice(0, 80))
          } catch (e) {
            actions.push({ tool: name, ok: false, err: e.message })
            console.log('[Wakeup/MCP] Error:', name, e.message)
          }
        } else {
          try {
    ...
    // 如果AI没返回文字，但调了schedule_wakeup，用其reason作为msg
    let finalMsg = wakeMsg
    if (!finalMsg && choice.message && choice.message.tool_calls) {
      const scheduleCall = choice.message.tool_calls.find(tc => tc.function && tc.function.name === 'schedule_wakeup')
      if (scheduleCall) {
        try {
          const args = JSON.parse(scheduleCall.function.arguments || '{}')
          finalMsg = args.reason || ''
        } catch {}
      }
    }
