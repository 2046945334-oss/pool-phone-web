    ...
    try {
      ...
      for (const t of tools) {
        ...
        if (t.name === 'lutopia_cli') {
          desc = `[MCP:${conn.name}] Lutopia 论坛命令行工具。
参数说明：
- command: 完整的命令行字符串，例如 "list --limit 10"（查看最近帖子）或 "inbox"（查看私信）
...
• 查看私信： "inbox"
• 发私信： "dm 对方名字 消息内容"
• 查看自己活动： "activity --limit 10"

完整用法用 command="help" 查看。`
          
          // 强化 schema 约束
          if (schema.properties?.command) {
            schema.properties.command.description = '完整的命令行字符串（如 "list --limit 10" 或 "inbox"），不要只传子命令'
          }
        }
