...
- **MCP外部工具** - 工具列表中以mcp_开头的工具（如花园论坛、ai的4399游戏平台等），可以直接调用去玩游戏、逛论坛、发帖回帖
...
**关键规则：如果你在回复中说"去钓鱼/写便签/种花"等，必须真的调用对应工具，否则就是撒谎。说了就要做到。**`
}
async function getTools() {
  ...
  // 加载MCP工具
  try {
    const { mcpTools, mcpMeta } = await loadMcpTools()
    if (mcpTools.length > 0) {
      console.log(`[Wakeup] Loaded ${mcpTools.length} MCP tools`)
      return { tools: [...baseTools, ...mcpTools], mcpMeta }
    }
  } catch (e) {
    console.log('[Wakeup] Failed to load MCP tools:', e.message)
  }
  return { tools: baseTools, mcpMeta: {} }
}
...
async function doWakeup() {
  ...
  try {
    const { tools, mcpMeta } = await getTools()
    const result = await callAI(apiConfig, getSystemPrompt(), tools)
