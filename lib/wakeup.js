      ...
      for (const tc of choice.message.tool_calls) {
        ...
        if (name === 'schedule_wakeup') {
          ...
          try {
            ...
            // 提取AI的文字回复（如果有的话）并附加到留言后
            const wakeMsg = ((choice.message && choice.message.content) || '').trim()
            const finalText = wakeMsg ? `${args.text || '...'}\n\n${wakeMsg}` : (args.text || '...')
            inbox.push({ role: 'assistant', content: finalText, ts: Date.now() })
            ...
            try {
              ...
              const duplicate = chatHistory.some(m => m && m.role === 'assistant' && m.content === finalText && m.wakeupMessage)
              if (!duplicate) {
                chatHistory.push({ role: 'assistant', content: finalText, ts: Date.now(), wakeupMessage: true })
