    ...
    while ((toolMatch = toolRegex.exec(wakeMsg)) !== null) {
      try {
        ...
        if (name === 'schedule_wakeup') {
          ...
          try { await callMusicApi('GET', '/music/search?keyword=' + encodeURIComponent(args.keyword || '')); actions.push({ tool: name, ok: true }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
          ...
          try { await callMusicApi('GET', '/music/playlist'); actions.push({ tool: name, ok: true }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
          ...
          try { await callMcpToolDirect(mcpMeta[name], args); actions.push({ tool: name, ok: true }) } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        } else if (executeToolFn) {
          try { await executeToolFn(name, args); actions.push({ tool: name, ok: true }); console.log('[Wakeup]', name, 'ok') } catch (e) { actions.push({ tool: name, ok: false, err: e.message }) }
        }
      } catch (parseErr) {
