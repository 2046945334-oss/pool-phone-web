...
export async function pullAllFromBackend() {
  // Keys that are managed by their own app's syncFromBackend — skip to avoid race conditions
  const pullSkipKeys = new Set([
    'pool_fishing_v2', 'pool_chat_history', 'pool_wake_inbox', 'pool_wake_log',
    'pool_ai_wakeup_state', 'pool_ai_status'
  ])
  try {
    ...
    for (const row of keys) {
      ...
      if (pullSkipKeys.has(key)) continue
      try {
...
export async function pushAllToBackend() {
  const syncPrefixes = ['pool_', 'f_', 'study', 'radio_', 'mail_', 'voice_']
  const skipKeys = new Set([
    ...
    'pool_ai_wakeup_state', 'pool_ai_status', 'pool_pixel_garden', 
    'pool_couple_space_v2', 'pool_notes_v3', 'pool_diary', 'pool_moments',
    'pool_reader_books', 'pool_reader_state', 'pool_reader_bookmarks', 'pool_reader_notes', 'pool_reader_chat',
