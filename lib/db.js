...
// 数据目录：优先环境变量，其次检测 /data 是否可用（Zeabur 持久卷），最后用项目 .data
function resolveDataDir() {
  if (process.env.DATA_DIR) return process.env.DATA_DIR
  // Always try /data first (Zeabur persistent volume) regardless of NODE_ENV
  try {
    if (!fs.existsSync('/data')) fs.mkdirSync('/data', { recursive: true })
    // Test write access
    const testFile = '/data/.write_test_' + process.pid
...
console.log('[db] DATA_DIR =', DATA_DIR, '| NODE_ENV =', process.env.NODE_ENV, '| cwd =', process.cwd())
...
function getDb() {
  if (!db || !db.open) {
    ...
    // Use DELETE journal mode for container durability.
    // WAL mode stores uncommitted data in a separate -wal file which can be
    // lost when containers restart (Zeabur re-deploys). DELETE mode writes
    // directly to the main .db file, so data survives container restarts.
